...
impl FMState {
    ...
    pub fn get_idx(&mut self) -> Option<usize> {
        if let Some(focused) = &self.focused {
            self.list_current()
                .iter()
                .position(|direle| &direle == &focused)
        } else {
            None
        }
    }
    ...
    pub fn jump_to(&mut self, new_focused: PathBuf) -> Option<usize> {
        if new_focused.is_dir() {
            self.current_dir = new_focused.to_path_buf();
            Some(0)
        } else {
    ...
    fn format_command(&self, cmd: &mut String) -> Option<Vec<String>> {
        ...
        if new_cmd.contains("%F") || new_cmd.contains("%D") {
            ...
            commands.push(new_cmd.to_string());
        }
        Some(commands)
    }

    fn execute_one_cmd(cmd: &String) -> Option<()> {
